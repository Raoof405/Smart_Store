...
export default function EditeTaskInProgress({ id }: PropsType) {
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [product, setProduct] = useState<Product | null>(null);
  // /+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  const [selection1, setSelection1] = useState<categorie[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const response = await axiosIns.post(`/dashboard/product/update/${id}`, {
        ...product,
        category_id: Number(product?.category_id),
        min_stock: Number(product?.min_stock),
        price: Number(product?.price),
        sector_id: Number(product?.sector_id),
  ...
  const fetchSel1 = async () => {
    try {
      const response = await axiosIns.get<{ data: categorie[] }>(
        ...
        <form onSubmit={handleSubmit}>
          ...
          <DialogContent>
            <Box className="grid grid-cols-12" paddingY={2} gap={2}>
              ...
              />
              {/* <FormControl className="col-span-12 md:col-span-4" fullWidth>
                <InputLabel id="demo-simple-select-label">رقم الرف </InputLabel>
                <Select
                  name="sector_id"
                  type="number"
                  className="col-span-12 md:col-span-4"
