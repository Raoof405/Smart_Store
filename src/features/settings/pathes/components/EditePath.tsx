...
export default function EditePath({ id }: PropsType) {
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    setLoading(true);
    axiosIns
      .post(`/dashboard/corridor/update/${id}`, formData)
      .then((res) => {
    ...
    <>
      ...
      <Dialog maxWidth="sm" fullWidth open={open}>
        <form onSubmit={handleSubmit}>
          <DialogTitle>تعديل الممر</DialogTitle>
          <DialogContent>
            <Box className="grid grid-cols-3" paddingY={2} gap={2}>
              <TextField
                required
                name="label"
                type="name"
