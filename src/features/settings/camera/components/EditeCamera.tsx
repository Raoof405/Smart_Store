...
export default function EditeCamera({ id }: PropsType) {
  const [cam, setCam] = useState<Camera | null>(null);
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  // /+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  const [selection1, setSelection1] = useState<Corridor[]>([]);
    ...
    const fetchProduct = async () => {
      try {
        const response = await axiosIns.get<ApiResponse>(
          `/dashboard/camera/show/${id}`
        );
        setCam(response.data.data);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    axiosIns
      .post(`/dashboard/camera/update/${id}`, {
  ...
  const handleChangeselction1 = (event: SelectChangeEvent<string | number>) => {
    setCorridorId(event.target.value as number);
  };
  return (
    <>
      ...
      <Dialog maxWidth="sm" fullWidth open={open}>
        <form onSubmit={handleSubmit}>
          <DialogTitle>تعديل كاميرا</DialogTitle>
          <DialogContent>
